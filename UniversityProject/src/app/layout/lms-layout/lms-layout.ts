    ...
    <div class="min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300 flex">
      ...
      <aside [ngClass]="{'w-64': isSidebarOpen(), 'w-20': !isSidebarOpen()}" 
        ...
        <div class="h-20 flex items-center justify-center border-b border-slate-200 dark:border-slate-800 px-4 shrink-0">
          <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-600 to-fuchsia-600 flex items-center justify-center text-white font-bold text-xl shadow-lg shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m22 10-7.5 4.13a2.5 2.5 0 0 1-2.38 0L2 10l9.31-5.12a2.5 2.5 0 0 1 2.38 0Z"/><path d="M6 12.2v4.5a2.5 2.5 0 0 0 1.25 2.16l3.56 2.05a2.5 2.5 0 0 0 2.38 0l3.56-2.05a2.5 2.5 0 0 0 1.25-2.16v-4.5"/></svg>
          </div>
          <div *ngIf="isSidebarOpen()" class="ml-3 flex flex-col animate-fade-in whitespace-nowrap overflow-hidden">
            <span class="font-black text-lg text-slate-900 dark:text-white leading-tight">LMS Core</span>
            <span class="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">Learning Platform</span>
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto py-6 px-3 custom-scrollbar">
          <div class="space-y-8">
            <!-- MAIN -->
            <div>
              <p *ngIf="isSidebarOpen()" class="px-3 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3">Learning</p>
              <ul class="space-y-1">
                <li>
                  <a routerLink="/lms/dashboard" routerLinkActive="bg-violet-50 text-violet-600 dark:bg-violet-500/10 dark:text-violet-400 font-bold" class="flex items-center px-3 py-2.5 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-white transition-all group">
                ...
                <li>
                  <a routerLink="/lms/courses" routerLinkActive="bg-violet-50 text-violet-600 dark:bg-violet-500/10 dark:text-violet-400 font-bold" class="flex items-center px-3 py-2.5 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-white transition-all group">
            ...
            <!-- INTERACTIVE -->
            <div>
              <p *ngIf="isSidebarOpen()" class="px-3 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3">Interactive</p>
              <ul class="space-y-1">
                <li>
            ...
            <!-- PROGRESS -->
            <div>
              <p *ngIf="isSidebarOpen()" class="px-3 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3">Tracking</p>
              <ul class="space-y-1">
                <li>
                  <a href="#" class="flex items-center px-3 py-2.5 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-white transition-all group">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="shrink-0 group-hover:scale-110 transition-transform"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
      ...
      <div class="flex-1 flex flex-col min-w-0 transition-all duration-300" [class.ml-0]="!isSidebarOpen()">
        ...
        <header class="h-20 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 sticky top-0 z-30">
          <div class="flex items-center gap-4">
            ...
            <div class="hidden sm:flex items-center bg-slate-100 dark:bg-slate-800 rounded-full px-4 py-2 w-64 border border-transparent focus-within:border-violet-500 focus-within:bg-white dark:focus-within:bg-slate-900 transition-all">
              ...
              <input type="text" placeholder="Search courses, skills..." class="bg-transparent border-none outline-none text-sm ml-2 w-full text-slate-700 dark:text-slate-300">
            </div>
          </div>
            ...
            <div class="relative pl-4 border-l border-slate-200 dark:border-slate-800">
              <div (click)="toggleDropdown($event)" class="flex items-center gap-3 cursor-pointer p-1 pr-4 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors border border-transparent">
                <div class="text-right hidden sm:block">
                  <p class="text-sm font-bold text-slate-900 dark:text-white leading-none">{{ user()?.name || 'Student Learner' }}</p>
                  <p class="text-xs text-slate-500 mt-1 font-medium">{{ user()?.role || 'Pro Member' }}</p>
                </div>
                <div class="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-700 border-2 border-violet-500 overflow-hidden hover:ring-2 hover:ring-violet-500/30 transition-all">
                  <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Learner" alt="Avatar" class="w-full h-full object-cover" />
                </div>
              ...
              <div *ngIf="isDropdownOpen()" (click)="$event.stopPropagation()" class="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-900 rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.15)] dark:shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)] border border-slate-200/50 dark:border-slate-800/50 py-2 animate-fade-in-up z-50 backdrop-blur-xl">
                <div class="px-4 py-3 border-b border-slate-100 dark:border-slate-800 mb-1">
                  <p class="text-sm font-bold text-slate-900 dark:text-white truncate">{{ user()?.name || 'Student Learner' }}</p>
                  <p class="text-xs text-slate-500 truncate mt-0.5">{{ user()?.email || 'lms@smartuni.edu' }}</p>
                </div>
                
                <a routerLink="/lms/profile" class="flex items-center px-4 py-2.5 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-violet-600 dark:hover:text-violet-400 transition-colors">
